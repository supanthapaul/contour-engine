...
import org.supanthapaul.util.JMath;
...
public class DebugDraw {
    ...
    public static void addBox2D(Vector2f center, Vector2f dimensions, float rotation, Vector3f color, int lifetime) {
        // calculate bottom left and top right vertices
    ...
    public static void addCircle(Vector2f center, float radius, Vector3f color, int lifetime) {
        Vector2f[] points = new Vector2f[18];
        int increment = 360 / points.length;
        int currentAngle = 0;

        // calculate circumference points of the circle
        for(int i = 0; i < points.length; i++) {
            Vector2f tmp = new Vector2f(radius, 0);
            JMath.rotate(tmp, currentAngle, new Vector2f());
            points[i] = new Vector2f(tmp).add(center);

            // add line for each segment of the circle
            if(i > 0) {
